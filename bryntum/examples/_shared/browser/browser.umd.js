...
function () {
  function ExamplesApp() {
    ...
    var me = this,
        product = bryntum.scheduler.StringHelper.capitalizeFirstLetter(shared.productName),
        version = bryntum.scheduler.VersionHelper.getVersion(shared.productName);
    ...
    if (location.hash) {
      ...
      if (exampleId) {
        ...
        if (exampleThumb) {
          var scrollable = new bryntum.scheduler.Scroller({
    ...
    value: function render() {
      ...
      bryntum.scheduler.WidgetHelper.attachTooltip(examplesThumbsContainerEl, {
        ...
        getHtml: function getHtml(_ref) {
          ...
          if (!me.currentTipLoadPromiseByURL[url]) {
            var requestPromise = me.currentTipLoadPromiseByURL[url] = bryntum.scheduler.AjaxHelper.get(url, {
              parseJson: true
            });
      ...
      bryntum.scheduler.EventHelper.on({
        ...
        click: function click(event) {
          var el = bryntum.scheduler.DomHelper.up(event.target, '[data-popup]');
          new bryntum.scheduler.Popup({
            forElement: el,
    ...
    value: function embedDescriptions() {
      ...
      return new Promise(function (resolve) {
        var me = _this2,
          ...
          return bryntum.scheduler.AjaxHelper.get(example.folder + '/app.config.json', {
            parseJson: true
